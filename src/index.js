...
let NodeHookAddon = require('bindings')('uiohook_napi');
...
class IOHook extends EventEmitter {
     ...
     */
    load() {
        NodeHookAddon.start(this._handler.bind(this), this.debug || false);
    }
    ...
    _handler(msg) {
        console.log(msg)
        if (this.active === false || !msg) return console.log('return');
        if (events[msg.type]) {
            const event = JSON.parse(JSON.stringify(msg))

            // const event = msg // 这里是复制给event然后操作
            event.type = events[msg.type]; // 这里难道是对应类似于 给const变量重新赋值的c++错误吗
            // delete event.type
            // console.log(msg)
